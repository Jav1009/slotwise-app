...
//
// Changes:
//   • Reads res.data['data'] (new consistent response shape)
//   • Shows customer name + email on each card
//   • Tap card → shows booking detail in bottom sheet
...
class _ManageBookingsScreenState extends State<ManageBookingsScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      if (_filterDate   != null) params['date']   = _filterDate!;

      final res    = await _api.get(ApiConstants.bookings, params: params.isNotEmpty ? params : null);
      final list = (res.data is Map ? res.data['data'] : res.data) as List;
      setState(() {
        _bookings = list.map((j) => BookingModel.fromJson(j)).toList();
        _loading  = false;
      });
    } catch (e) {
  ...
  Future<void> _updateStatus(BookingModel b, String newStatus) async {
    ...
    if (mounted) {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Update failed'), backgroundColor: AppColors.danger));
      }
    }
  }
      ...
      builder: (_) => Padding(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          if (b.notes != null && b.notes!.isNotEmpty)
            _row(Icons.notes, 'Notes', b.notes!),
        ]),
      ),
    );
  }
